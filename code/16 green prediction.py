...
while count < 7:
    ...
    for chunk in pd.read_csv("dataset/g0"+str(count)+".csv", chunksize=10000):
        ...
        pool = chunk[['Passenger_count','Trip_distance', 'Extra', 'MTA_tax',
                      ...
                      'Total_amount', 'Payment_type','Trip_type ']].copy()
...
plt.title('Green Actual vs Fitted Values for tip amount')
plt.legend()
plt.savefig('Green Actual vs Fitted Values for tip amount (Credit Card).png')
plt.show()
plt.close()